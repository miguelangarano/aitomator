...
export async function loadAItomatorConfig(from = process.cwd()): Promise<AItomatorConfig> {
  ...
  const defaults: AItomatorConfig = { workspace, database: join(workspace, "data", "aitomator.db"), http: { host: "127.0.0.1", port: 8787 }, concurrency: { maxRuns: 2, defaultWorkflowMaxRuns: 1 }, logging: { level: "info" } }
  ...
  const result: AItomatorConfig = { ...defaults, ...configured, workspace: configured.workspace ? resolveWorkspacePath(workspace, configured.workspace) : workspace, http: { ...defaults.http, ...configured.http }, concurrency: { ...defaults.concurrency, ...configured.concurrency }, logging: { ...defaults.logging, ...configured.logging }, database: configured.database ? resolveWorkspacePath(workspace, configured.database) : defaults.database }
