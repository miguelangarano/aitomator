...
import { appendExecutionLog, executionLogPath, initializeExecutionLog, legacyExecutionLogPath } from "../logging/execution"
...
async function runs(): Promise<void> {
  ...
  else if (sub === "retry") { const original = getRun(db, args[1] ?? ""); if (!original) fail("RUN_NOT_FOUND", `Run not found: ${args[1] ?? ""}`, 3); const record = retryRun(db, original); initializeExecutionLog(config.workspace, record.workflow_id, record.id); appendExecutionLog(config.workspace, record.workflow_id, record.id, "INFO", "cli", `Execution queued as retry of ${original.id}`); print(json ? publicRun(record) : `Queued retry ${record.id}`, json) }
...
async function logs(): Promise<void> { const config = await loadAItomatorConfig(); if (hasFlag(args, "--daemon")) { const code = await showServiceLogs(config.workspace, hasFlag(args, "--follow"), Number(option(args, "--lines") ?? 100)); if (code) process.exitCode = code; return } const runId = option(args, "--run"), workflowId = option(args, "--workflow"); if (hasFlag(args, "--follow")) return followWorkflowLogs({ workspace: config.workspace, database: config.database, runId, workflowId, lines: Number(option(args, "--lines") ?? 100), initialRuns: Number(option(args, "--limit") ?? 20) }); const db = openDatabase(config.database); const records = runId ? [getRun(db, runId)].filter(Boolean) : listRuns(db, Number(option(args, "--limit") ?? 20), workflowId); if (runId && records.length === 0) { db.close(); fail("RUN_NOT_FOUND", `Run not found: ${runId}`, 3) } const content = records.map(record => { const current = executionLogPath(config.workspace, record!.workflow_id, record!.id), legacy = legacyExecutionLogPath(config.workspace, record!.id), path = existsSync(current) ? current : legacy; return existsSync(path) ? readFileSync(path, "utf8") : "" }).join(""); db.close(); process.stdout.write(content || "No logs found\n") }
